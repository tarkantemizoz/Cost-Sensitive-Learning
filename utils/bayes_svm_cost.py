...
class bayes_svm_cost: 
    """ Hyperparameter optimisation for Cost Sensitive Support Vector Machines using Bayesian Optimization.
    ...
    def train_bayes_svm_cost(self, trial):
        ...
        space = {'C': trial.suggest_int('C', -10, 10)}
        ...
        for train_index, test_index in skf.split(self.data, self.labels):
            ...
            svm_model = SVM("linear", C = 2 ** space['C'])
            svm_model.fit(x_train, y_train, r_train)
    ...
    def bayes(self):
        ...
        study.optimize(func=self.train_bayes_svm_cost, n_trials=self.bayes_trials)
