...
class bayes_logistic: 
    ...
    def train_bayes_logistic(self, trial):
        ...
        space = {'c': trial.suggest_loguniform("c", 1e-7, 10.0),
        ...
        for train_index, test_index in skf.split(self.data, self.labels):
            ...
            if space["penalty"] != "elasticnet":
                clf = LogisticRegression(C = space['c'], penalty = space["penalty"], solver='saga').fit(x_train, y_train)
            else:
                clf = LogisticRegression(C = space['c'], penalty = space["penalty"], solver='saga', l1_ratio = space['l1']).fit(x_train, y_train)                   
